...
#define FUNCTIONS_TIME (250)

#define ACTUAL_SECOND (SECOND-FUNCTIONS_TIME)
...
void TMR0_Update(void) __interrupt 0 // @ 5 ms
{
    ...
    /* Tasks */
    if (counter % (4*TMR0_TICK_MS)==0)
    {
        PB_Update(); // @ 20 ms
        CNT_Update(); // @ 20 ms
    }
    /* toggle led every half second */
    if(counter % (ACTUAL_SECOND/2) == 0){
        CNT_toggleLed();
    }
    /* increment seconds then reset counter every second */
    if(counter == ACTUAL_SECOND) {
        CNT_IncSeconds();
        counter=0;
    }
    SSD_Update();
